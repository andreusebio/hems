...
void EnergyMonitor::calcVI(unsigned int crossings, unsigned int timeout)
{
  ...
  {
    startV = (ads.readADC_SingleEnded(inPinV))/1.6119257;                    //using the voltage waveform
  ...
  while ((crossCount < crossings) && ((millis()-start)<timeout))
  {
    ...
    //-----------------------------------------------------------------------------
    sampleV = (ads.readADC_SingleEnded(inPinV))/1.6119257;                 //Read in raw voltage signal
    sampleI = (ads.readADC_SingleEnded(inPinI))/1.6119257;                 //Read in raw current signal
...
double EnergyMonitor::calcIrms(unsigned int Number_of_Samples)
{
  ...
  {
    sampleI = (ads.readADC_SingleEnded(inPinI))/1.6119257;
